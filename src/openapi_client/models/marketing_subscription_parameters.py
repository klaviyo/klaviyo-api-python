...
class MarketingSubscriptionParameters(BaseModel):
    ...
    consented_at: Optional[datetime] = Field(default=None, description="The timestamp of when the profile's consent was gathered. This should only be used when syncing over historical consent info to Klaviyo; if not included, the current time will be used. This field is only supported for single opt-in and will be ignored for double opt-in.")
